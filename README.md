...
The following has to be added in the Start G-Code (`;Slicer info:<key>;<value>`). You can add as much metadata as you like and decide to view the metadata in the filelist or printer state. **Systax changed from version 1.3.0 as the description  is maintanined in settings now but old styles are sill supported.**

Example for Cura 4.12 and newer (For Anycubic Mega S, Pro and X it is available by default in the default Start G-Code.)
